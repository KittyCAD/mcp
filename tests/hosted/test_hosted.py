...
from zoo_mcp.hosted.projects import Projects
...
async def test_complete_source_and_fresh_job_download_links(monkeypatch):
    ...
    runtime = Runtime(backend)
    p = backend.owner
    source = await Projects(backend).write_source(
...
async def test_catalog_preserves_tools_without_exposing_filesystem():
    ...
    assert hosted["open_zoo_workspace"].meta is not None
    assert hosted["import_attachment"].meta is not None
    assert hosted["open_zoo_workspace"].meta["ui"]["resourceUri"].startswith("ui://")
    assert hosted["import_attachment"].meta["openai/fileParams"] == ["file"]
...
def test_modern_http_routing_and_ui_resource():
    ...
    with TestClient(create_app(backend=backend), base_url="https://mcp.test") as client:
        ...
        assert response.json()["result"]["structuredContent"]["workspace_url"].endswith(
            "/mcp/workspace"
        )
    ...
    with TestClient(app, base_url="https://mcp.test") as client:
        ...
        response = client.post(
            ...
            json={
                ...
                "id": 2,
                "method": "tools/call",
                "params": {"name": "open_zoo_workspace", "arguments": {}},
            },
        )
        assert response.status_code == 200 and "workspace_url" in response.text
        assert (
            client.post(
                "/mcp/workspace/call",
                headers=headers,
        ...
        assert client.get("/mcp/workspace").status_code == 200
        assert "Content-Security-Policy" in client.get("/mcp/workspace").headers
