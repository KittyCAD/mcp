...
def create_app(
    ...
    async def dispatch(p: Principal, name: str, arguments: dict) -> dict:
        ...
        if name == "write_kcl_project":
            return await projects.write_source(p, arguments["files"])
        ...
        if arguments.get("kcl_code") and name in {
            ...
            source = await projects.write_source(p, {"main.kcl": arguments["kcl_code"]})
    ...
    async def call(
        ...
        p.require(*scopes_for(name))
        if background(name) and not forwarded:
            return await runtime.submit(
                p, name, arguments, lambda: dispatch(p, name, arguments)
            )
        return await dispatch(p, name, arguments)
    ...
    async def call_tool(ctx, params) -> CallToolResult:
        try:
            ...
            # durable JSON result used for job recovery and the interactive app.
            for artifact in (
            ...
            return CallToolResult(
                ...
                content=[
                    TextContent(
                        type="text",
                        text="Zoo is temporarily unavailable. Retry the same operation key to recover its status.",
                    )
                ],
            )
    ...
    server = Server(
        ...
        instructions=(
            ...
            "Mutating and billable operations require an idempotency_key; get_job retrieves pending results. "
            "Restore expired scenes explicitly from saved source. open_zoo_workspace provides uploads and a 3D viewer."
        ),
    ...
    async def metrics(request: Request):
        ...
        lines.extend(
            f'zoo_mcp_job_outcomes_total{{outcome="{key}"}} {value}'
