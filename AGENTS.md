...
- `src/zoo_mcp/__init__.py` - Package initialization: configures logging, provides the loop-local `AsyncKittyCAD` client factory used across the modules, and defines `ZooMCPException` and `ZooMCPTimeoutError`.
