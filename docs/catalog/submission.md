...
Planned production MCP URL: `https://api.zoo.dev/mcp`.
Authentication: OAuth authorization code + PKCE S256, CIMD public clients.
Planned production workspace: `https://api.zoo.dev/mcp/workspace`.
...
Production enablement and catalog publication are separate rollout steps. These
materials do not enable production or submit a catalog entry. Finish the
public-preview exercises below before enabling the planned production URLs.
...
OAuth client documents, and finish the public-preview exercises in `docs/hosted.md`.
